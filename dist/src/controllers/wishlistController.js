...
export const getWishlist = async (req, res) => {
    try {
        ...
        const availableWishlist = user.wishlist.filter((room) => room && room.isAvailable !== false);
        res.status(200).json({
            success: true,
...
export const removeFromWishlist = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { $pull: { wishlist: roomId } }, { new: true }).populate({
            path: 'wishlist',
            select: 'roomTitle location price beds bathrooms type images amenities views isAvailable'
        });
        if (!user) {
