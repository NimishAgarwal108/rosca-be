...
const updateRoomLogic = async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.amenities) {
      if (typeof req.body.amenities === 'string') {
        try {
          ...
          amenitiesArray = req.body.amenities.split(',').map((a: string) => a.trim());
        }
    ...
    if (updatePayload.beds) updatePayload.beds = Number(updatePayload.beds);
    if (updatePayload.bathrooms) updatePayload.bathrooms = Number(updatePayload.bathrooms);
